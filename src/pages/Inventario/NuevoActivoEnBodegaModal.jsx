...
export default function NuevoActivoEnBodegaModal({ open, onClose, idBodega, onSaved }) {
    const { userData } = useAuth();
    const { showToast } = useToast();

    const [form, setForm] = useState({ codigo: "", nombre: "", modelo: "", serial_number: "", tipo: "Otro", estatus: "Activo" });
    ...
    async function onSubmit(e) {
        ...
        setSaving(true);
        try {
            await createActivoEnBodega({ ...form, id_bodega: idBodega, usuario_responsable: userData?.id_usuario });
        ...
        <Modal open={open} onClose={onClose}>
            <ModalDialog component="form" onSubmit={onSubmit} sx={{ width: { xs: "100%", sm: 520 } }}>
                ...
                <Stack spacing={1.5} mt={1}>
                    <FormControl required><FormLabel>Código</FormLabel>
                        <Input value={form.codigo} onChange={e => setForm({ ...form, codigo: e.target.value })} />
                    </FormControl>
                    <FormControl required><FormLabel>Nombre</FormLabel>
                        <Input value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value })} />
                    </FormControl>
                    <FormControl><FormLabel>Modelo</FormLabel>
                        <Input value={form.modelo} onChange={e => setForm({ ...form, modelo: e.target.value })} />
                    </FormControl>
                    <FormControl><FormLabel>Serie</FormLabel>
                        <Input value={form.serial_number} onChange={e => setForm({ ...form, serial_number: e.target.value })} />
                    </FormControl>
                    <FormControl><FormLabel>Tipo</FormLabel>
                        <Select value={form.tipo} onChange={(_, v) => setForm({ ...form, tipo: v })}>
                            {TIPOS.map(t => <Option key={t} value={t}>{t}</Option>)}
                        </Select>
                    </FormControl>
                    <FormControl><FormLabel>Estatus</FormLabel>
                        <Select value={form.estatus} onChange={(_, v) => setForm({ ...form, estatus: v })}>
